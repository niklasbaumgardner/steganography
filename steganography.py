...
def encode_image(image, bi_string):
    #print(bi_string, len(bi_string))
    im, image = open_image(image) #Image.open(image)
    w, h = im.size
    pix = im.load()
    count = 0
    start = time.time()
...
def main():
    ...
    if user_input == 't':
        print('Time test')
        txt_file = 'beemoviescript.txt'
        message = readfile(txt_file)
        b_string = encode_string(message)
        avg = []
        #for _ in range(5):
        #start = time.time()
        encode_image('Valley.jpg', b_string)
        #end = time.time()
        #avg.append(end - start)
        #print('Time for PIL:', sum(avg) / len(avg))
        return
